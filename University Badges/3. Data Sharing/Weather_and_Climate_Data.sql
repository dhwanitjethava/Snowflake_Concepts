...
-- When it snows in excess of six inches per day, my company experiences delivery delays.
-- How many of my deliveries were impacted during the third week of January for the previous year?
...
-- I am hosting an outdoor event in seven days. How can I utilize your forecast data to determine
-- if my event will be impacted by rain?
...
-- Alter database name to WeatherSource database
...
create or replace view DEMO_DB.PUBLIC.DETROIT_ZIPS (zip_code)
as select distinct postal_code from WEATHERSOURCE.STANDARD_TILE.HISTORY_DAY
    where postal_code like '481%' or postal_code like '482%';

-- Count the number of rows in the HISTORY_DAY view.
select count(*) from WEATHERSOURCE.STANDARD_TILE.HISTORY_DAY;

-- Count the number of rows after joining the HISTORY_DAY view with the DETROIT_ZIPS view 
select count(*) from WEATHERSOURCE.STANDARD_TILE.HISTORY_DAY hd
join DEMO_DB.PUBLIC.DETROIT_ZIPS dz
on hd.postal_code = dz.zip_code;

-- What's the Data Range on this Data Set for HISTORY view?
select max(date_valid_std), min(date_valid_std) from history_day hd
join demo_db.public.detroit_zips dz
on hd.postal_code = dz.zip_code;
-- Looks like the HISTORY view has two years of data

-- What's the Data Range on this Data Set for FORECAST view?
select max(date_valid_std), min(date_valid_std) from forecast_day fd
join demo_db.public.detroit_zips dz
on fd.postal_code = dz.zip_code;
-- Looks like the FORECAST view has two weeks of data

-- Which day in next 2 weeks would be best for a sale?
select date_valid_std, avg(avg_cloud_cover_tot_pct) from forecast_day fd
join demo_db.public.detroit_zips dz
on fd.postal_code = dz.zip_code
